...
<div class="filament-apex-charts-header">
    ...
    <div class="sm:flex justify-between gap-4 py-2 relative">
        ...
        <div>
            @if ($heading)
            <x-filament::section class="filament-apex-charts-heading">
                {!! $heading !!}
            </x-filament::section>
            @endif
        ...
        <div>
            @if ($filters)
            <select wire:model="filter" @class([ 'apex-charts-single-filter w-full text-gray-900 border-gray-300 block h-10 transition duration-75 rounded-lg shadow-sm focus:border-primary-500 focus:ring-1 focus:ring-inset focus:ring-primary-500' , 'dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:focus:border-primary-500'=> config(
                'filament.dark_mode'),
